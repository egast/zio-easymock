  ...
  override def spec =
    suite("zioeasymock")(
      ...
      testM("test strict mocking") {
        createStrictMock[TestService.Service].expecting { service1 =>
          ...
          assertM(
            TestService.doSomething(1000) *> TestService.doSomething(2000) *>
              TestService.doSomething(3000)
          )(
            equalTo("3000")
          ).provideCustomLayer(mockLayer)
        )
      ...
      testM("test mocking with capture") {
        for {
          ...
          result  <- expecting[TestService.Service, TestService2.Service] { (service1, service2) =>
                       expect(service1.doSomething(EasyMock.capture(capture)))(
                         _.andReturn(ZIO.effect(capture.getValue.toString))
                       ) *>
                         expect(service2.doSomething2(200))(_.andReturn(ZIO.effectTotal("200")))
      ...
      testM("test mocking create mock failure") {
        ...
        }.whenExecuting(string => assertM(ZIO.effect(string.charAt(100)))(equalTo('e')))
      } @@ failing,
      ...
      testM("test nested mocking ") {
        expecting[TestService.Service] { service1 =>
          ...
          expecting[TestService2.Service] { service2 =>
            ...
            assertM(
              TestService2.doSomething2(200) *> TestService.doSomething(1000) *>
                TestService.doSomething(2000)
            )(
              equalTo("2000")
      ...
      testM("test mocking") {
        checkM(Gen.listOf(Gen.anyInt)) { numbers =>
          expecting[TestService.Service] { service1 =>
            ZIO.foreach(numbers)(n => expect(service1.doSomething(n))(_.andReturn(ZIO.effectTotal(n.toString))))
          }.whenExecutingAsLayer(mockLayer =>
            assertM(
