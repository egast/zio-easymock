...
case class Mocks1[A: Tagged](mock1: Task[A]) {
  ...
  def expecting(expectation: A => Task[IExpectationSetters[_]]): ExpectingMocks1[A] =
...
case class Mocks2[A: Tagged, B: Tagged](mock1: Task[A], mock2: Task[B]) {
  def expecting(expectation: (A, B) => Task[IExpectationSetters[_]]): ExpectingMocks2[A, B] =
...
case class Mocks3[A: Tagged, B: Tagged, C: Tagged](mock1: Task[A], mock2: Task[B], mock3: Task[C]) {
  def expecting(expectation: (A, B, C) => Task[IExpectationSetters[_]]): ExpectingMocks3[A, B, C] =
...
case class ExpectingMocks1[A: Tagged](mocks: Mocks1[A], expectation: A => Task[IExpectationSetters[_]]) {
...
case class ExpectingMocks2[A: Tagged, B: Tagged](mocks: Mocks2[A, B], expectation: (A, B) => Task[IExpectationSetters[_]]) {
...
case class ExpectingMocks3[A: Tagged, B: Tagged, C: Tagged](mocks: Mocks3[A, B, C], expectation: (A, B, C) => Task[IExpectationSetters[_]]) {
...
private[easymock] object Mocking {
  ...
  (expectation: M => Task[IExpectationSetters[_]], assertion: M => ZIO[R, E, TestResult])
  (mocks: Task[M]) =
    mocks.foldM(
  ...
  private def testMocks[M <: Product : ClassTag, R, E](expectation: M => Task[IExpectationSetters[_]], assertion: M => ZIO[R, E, TestResult])(allMocks: M): ZIO[R, E, TestResult] =
    (for {
